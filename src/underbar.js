(function() {
  ...
  _.last = function(array, n) {
    ...
    if (n > lastIndex) {
      ...
      startPoint = lastIndex - n;
    }
    return n === undefined ? array[lastIndex - 1] : array.slice(startPoint, lastIndex);
  };
    ...
    if (arguments.length === 1) {
      ...
      for (let i = 1; i < array.length; i++) {
        ...
        for (let i = 0; i < array.length; i++) {
          ...
          if (iterator(array[i]) !== iterator(results[i - 1])) {
            results.push(array[i]);
          }
        }
      }
  ...
  _.shuffle = function(array) {
    ...
    while (indexArray.length < newArray.length) {
      var test = num();
      if (_.contains(indexArray, test) === false) {
        indexArray.push(test);
      }
    }

    var returnArray = [];
    for (var i = 0; i < newArray.length; i++) {
      returnArray.push(newArray[indexArray[i]]);
    }
